...
class QuestionaireListView(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if self.request.GET.get("medical"):
            queryset=queryset.filter(medical_condition__contains=[self.request.GET.get("medical")])
        if  self.request.GET.get("symptoms"):   
            queryset=queryset.filter(symptoms__contains=[self.request.GET.get("symptoms")])   
        return queryset
...
class BlogmainListView(ListView):
   
    model = Blogmain
    raise_exception = True
    template_name = 'catalogue/blogmain_list.html'
    success_url = reverse_lazy('dashboard:blogmain_list')
    paginate_by = 20
    
    def get_success_url(self):
        return reverse_lazy('dashboard:blogmain_list')
...
class BlogmainCreateView(CreateView):
    """
   blog Create View
    """
    template_name = 'catalogue/blogmain_form.html'
    model = Blogmain
    form_class: BlogMainForm
    raise_exception = True
    success_message = SUCCESS_MESSAGES['CUSTOMER_TESTIMONIAL_ADDED']
...
class BlogmainUpdateView(UpdateView):
...
def about(request,Frame1):
    template = "frame1_form.html"
    if request.method == "GET":
        Frame = request.GET.get("{{object.frames}}")
        Image = request.GET.get("{{object.image}}")
        Title = request.GET.get("{{object.title}}")
