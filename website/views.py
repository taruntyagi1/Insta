...
class DeffieicencyReportView(DetailView):    
    ...
    def get_recommended_product(self):
        ...
        rec_product = recommendation_data.get('recommendation')[0]
        prod_obj_list = Product.objects.browsable().filter(title__startswith=rec_product)
        return prod_obj_list[0]   
    ...
    def get_context_data(self, **kwargs):
        ...
        context['product_faqs'] = FAQSproduct.objects.filter(product=context['recommended_product']).order_by('-question')
...
class DeffieicencyReportPdfView(PDFTemplateView):
    ...
    cmd_options={'margin-top': 5, 'enable_local_file_access': True, 'orientation': 'portrait',}
    ...
    def get_context_data(self, **kwargs):
        ...
        context['recommended_product'] = self.get_recommended_product()
        context['prod']=settings.HOST_URL
        return context
...
class NewHomePageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['footer'] = Footer.objects.all().filter(is_active = True)
        context['blogmain'] = Blogmain.objects.all().filter(is_active = True)
        context['blog'] = Blog.objects.all().filter()
...
class NewProductListView(ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get('category_list'):
            get_category_list=self.request.GET.get('category_list').split(',')
            a=[]
            for i in get_category_list:
                if i=='':
                    ...
                    a.append(i) 
            list_string = list(map(int, a))
            queryset = queryset.filter(categories__id__in=list_string).distinct()
